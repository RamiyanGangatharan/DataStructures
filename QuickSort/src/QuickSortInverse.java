...
public class QuickSortInverse {
    public static void main(String[] args) {
        int[] arr = new int[]{3,6,4,5,7,1,2};
        System.out.println("Original: " + Arrays.toString(arr));
        quickSortInverse(arr,0,arr.length-1);
        System.out.println("Sorted: " + Arrays.toString(arr));
    }

    public static void quickSortInverse(int[] arr, int left, int right) {
        // make sure there is something still to sort (not a single element)
        if (left < right) {
            // partitionInverse the array
            int pivot = partitionInverse(arr, left, right);
            //recursively sort two partitions
            quickSortInverse(arr, left, pivot - 1);
            quickSortInverse(arr, pivot + 1, right);
        }
    }


    public static int partitionInverse(int[] arr, int left, int right) {
        ...
        while (i <= j) {
            ...
            // Changed the second signs for the pivot to sort from largest to smallest
