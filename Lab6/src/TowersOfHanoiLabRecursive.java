...
public class TowersOfHanoiLabRecursive {
     ...
     *
     * @param numberofDiscs the number of discs to move
     * @param source the source tower
     * @param destination the destination tower
     * @param auxiliary the auxiliary tower
     */
    public static void moveDiscs(int numberofDiscs, Tower source, Tower destination, Tower auxiliary) {
        // BASE CASE
        if (numberofDiscs == 1) { moveTopDisc(source, destination); }
            ...
            // Move the numberofDiscs - 1 discs from auxiliary to destination
            moveDiscs(numberofDiscs - 1, auxiliary, destination, source);
        }
    }
    ...
    public static void moveTopDisc(Tower s, Tower d) {
        ...
        if (topDestination == null || topSource.getWidth() < topDestination.getWidth()) {
            ...
            System.out.println(
                    "ERROR: Cannot move disc " + topSource.getWidth() + " onto smaller disc " +
                    topDestination.getWidth() + " in" + d.getName());
        }
    }
    ...
    public static Disc printDisc(Disc disc) {
        ...
        System.out.print(centerPad("|".repeat(disc.getWidth()), towerWidth)); // changed the - to |
