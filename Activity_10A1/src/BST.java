...
public class BST {
  ...
  private Node insertRecursive(Node node, int data) {
    /*
      if spot found, insert data as new node otherwise check
      if the data is less than or greater than the current value
  ...
  private boolean dfsSearchRecursive(Node node, int target) {
    if (node == null) { return false; } //if leaf reached, it is not found in this branch (our base case)
    if (node.data == target) { return true; } // Check if the current node contains the target value

    // Recursively search in the left or right subtrees based on value
    if (target < node.data) { return dfsSearchRecursive(node.left, target); }
    else { return dfsSearchRecursive(node.right, target); }
  }
  ...
  private Node deleteRecursive(Node node, int data) {
    ...
    if (data < node.data) { node.left = deleteRecursive(node.left, data); }
    else if (data > node.data) { node.right = deleteRecursive(node.right, data); }
    else {
      ...
      node.right = deleteRecursive(inOrderSuccessor, data);
    }
