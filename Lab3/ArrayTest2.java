...
public class ArrayTest2 {
    public static void main(String[] args)
    {
        ...
        int[] numbers = {5000, 10000, 50000, 100000};
        long o = 0;

        for (int i = 0; i < numbers.length; i++) {

            int numOfElements = numbers[i];
            ...
            System.out.println("QUICK         ARRAY      " + o + "         " + elapsedTime);

            start = System.currentTimeMillis();
            o = ComplexityLab.insertionSortArray(array1);
            end = System.currentTimeMillis();
            long elapsedTime5 = end - start;
            System.out.println("INSERTION      ARRAY       " + o + "         " + elapsedTime1);
    ...
    public static void populateArray(int[] arr, int min, int max) {
        ...
        for (int i = 0; i < arr.length; i++) { arr[i] = rand.nextInt((max - min) + 1) + min; }
    }

    public static void populateLinkedList(LinkedList<Integer> list, int min, int max, int size)  {
        Random rand = new Random();
        for (int i = 0; i < size; i++) { list.add(rand.nextInt((max - min) + 1) + min); }
    }
